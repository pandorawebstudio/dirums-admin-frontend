...
function Page() {
  ...
  const createAutomatedPage = () => {
    setError("")
    if(products?.length === 0 && title?.length === 0 && !slug?.length === 0) {
      setError('Page Name or Slug or Product has not been selected !')
    }
    const query = {
    ...
    fetch(`${API_URL}/api/product${strigifiedQuery}`)
      ...
      .then(data => {
        ...
        fetch(`${BASE_URL}/dashboard/pages/api`, {
          method: 'POST',
    ...
    const fetchProducts = () => {
      setIsLoading(true);
        ...
        .then(res => res.json())
        .then(data => {
            console.log(data.docs, "products displayed");
    ...
    <Suspense fallback={<Spinner className="h-12 w-12" />}>
      <Sidebar>
        ...
        <div className="flex flex-col my-8 justify-between gap-4">
          <Input label="Page name" onInput={setTitleAndSlug} />
          <Input
            label="Slug"
            value={slug}
            onInput={(e) => {
                            ...
                            <tr key={index} className='text-center border-2'>
                              <td>
                                <Checkbox
                                  ...
                                  onChange={(e) =>
                                    products?.some((item) => item === prop1.id)
                                      ? setProducts(
                                        products.filter(
                                          (item) => item !== prop1.id
                                        )
          ...
          <Input
            ...
            onInput={(e) => handleInputValidation(e.target.value, 70, setHeading)}
            maxLength={70}
          />
          <Typography className='text-sm my-2 text-green-500 font-bold'>Maximum heading length: {heading?.length} / 70 characters</Typography>
          <Input
            label="Subheading"
